    ...
    public class Monopoly
    {
        ...
        static public void PrintPieces(int oldPosition, int newPosition, Player player)
        {
            ...
            for (int i = 0; i < playersCount; i++)
            {
                ...
                {
                    if (players[i].Position == oldPosition) oldPieces[oldPiecesPtr++] = players[i].Piece;
...
#pragma warning restore IDE0059 // Ненужное присваивание значения
        ...
        static public void PrintOwner(Quartal quartal)
        {
            ...
            {
                if (position < 15) bgColor = ConsoleColor.DarkGreen;
        ...
        static public void PrintRealty(Player player)
        {
            ...
            else
            {
                ...
                Console.Write($"Пробел – Снести дом и получить {quartal.Pledge}$");
            }
            Console.SetCursorPosition(165, 55);
            if (musicMuted)
            {
                ...
                switch(Console.ReadKey().Key)
                {
                    ...
                    case ConsoleKey.Enter:
                        ...
                        else
                        {
                            if (quartal.Level != 5 && quartal.IsMonopoly == true)
                            {
                                if (player.Balance >= quartal.HouseCost)
                                {
                                    moneyPlayer.Position = TimeSpan.Zero;
        ...
        static public void BuyOrAuctionOrRent(Player player, Quartal quartal, int turn)
        {
            ...
            else
            {
                if (!quartal.IsMantaged)
                {
                    ...
                    PrintMenu($"Заплатите ренту: {rent}$ игроку {quartal.Owner.Name}", rent, player);
                    bool notEntered = true;
                    while (notEntered)
                    {
                        switch (Console.ReadKey(true).Key)
        ...
        static public bool ThrowDices(Player player, int turn)
        {
            ...
            if (dice1 == dice2)
            {
                ...
                if (doubles == 3)
                {
                    ...
                    Thread.Sleep(1500);
                    player.Arrest();
                    quartals[10].IncreaseVisitors();
                    PrintPieces(30, 10, player);
                    quartals[30].DecreaseVisitors();
                    doubles = 0;
                    return false;
                }
                else
                        ...
                        if (!player.IsPrisoned)
                        {
                            ...
                            switch (quartal.Special)
                            {
                                ...
                                case 5:
                                    ...
                                    quartals[player.Position].DecreaseVisitors();
                                    turnNotEnded = false;
                                    break;
                                case 6:
                                case 7:
        ...
        static public void Auction(Quartal quartal)
        {
            ...
            do
            {
                for (int i = 0; i < playersCount; i++)
                {
                    ...
                    if (participant.Balance < bid + 1) participant.CancelAuction();
                    ...
                    {
                        spaces = 20 - buyer.Name.Length;
                        left = Convert.ToInt32(Math.Ceiling(spaces / 2.0));
                        right = Convert.ToInt32(Math.Floor(spaces / 2.0));
                        for (int j = 0; j < left; j++) Console.Write(" ");
                        Console.Write($"{buyer.Name} {buyer.Piece}");
                        for (int j = 0; j < right; j++) Console.Write(" ");
                    }
                    Console.Write("║");
            ...
            if (buyer != null)
            {
                moneyPlayer.Position = TimeSpan.Zero;
                moneyPlayer.Play();
                quartal.SetOwner(buyer, bid);
                PrintOwner(quartal);

                spaces = 54 - buyer.Name.Length;
                left = Convert.ToInt32(Math.Ceiling(spaces / 2.0));
                right = Convert.ToInt32(Math.Floor(spaces / 2.0));
                ClearMenu();
                Console.SetCursorPosition(165, 50);
                ...
                Console.Write("║");
                for (int i = 0; i < left; i++) Console.Write(" ");
                Console.Write($"Продано: {buyer.Name}");
                for (int i = 0; i < right; i++) Console.Write(" ");
                Console.Write("║");
                Console.SetCursorPosition(165, 53);
                Console.Write("╚═══════════════════════════════════════════════════════════════╝");
        ...
        static public Quartal Chance(Player player, int turn)
        {
            ...
            switch (card)
            {
                ...
                case 1:
                    ...
                    quartals[position].DecreaseVisitors();
                    return null;
                case 2:
                    sadPlayer.Position = TimeSpan.Zero;
                    sadPlayer.Play();
        ...
        {
            string path = Directory.GetCurrentDirectory() + @"\MonopolySounds\";
            while (true)
            {
                try
                {
